...
两次事件的共同之处有：衰减率曲线为双峰甚至多峰，说明先后抵达的日冕物质抛射；恢复相衰减率回落非常缓慢，暴后一周多仍然远高于背景水平，即热层能量注入快、耗散慢。将Kp指数向后平移6个小时再进行散点统计，低Kp下衰减率集中度较高，而在高Kp下离散度极大，当Kp达到8–9时CHAMP的衰减率可以从−10 m/h跨到−17 m/h，GRACE躞FO也可以从−2.5 m/h拉至−6.0 m/h。这意味着Kp是三个小时内的全球平均指数，只能提示地磁活动正在剧烈，具体强弱程度如何，能量注入热层，还取决于日冕物质抛射持续时长、行星际磁场南向分量积分强度等信息。总而言之，本文建立了一套不依赖星载加速度计，仅凭精密轨道数据就可反演轨道衰减的方法，并且在第25太阳活动周峰年期间可直接用于低轨航天器的轨道预报、碰撞规避和寿命评估。
...
Common features of the two events include: decay rate curves exhibiting bimodal or even multimodal distributions, indicating successive coronal mass ejections; and a very slow decline in the decay rate during the recovery phase, remaining far above background levels for more than a week after the storm, indicating rapid thermospheric energy injection and slow dissipation. When the Kp index is shifted backward by 6 hours for scatter plot analysis, the concentration of decay rates is higher at low Kp values, while the dispersion is extremely high at high Kp values. When Kp reaches 8–9, the CHAMP decay rate can range from −10 m/h to −17 m/h, and the GRACE FO decay rate can range from −2.5 m/h to −6.0 m/h. This implies that Kp, as a global average index over a three-hour period, can only indicate that geomagnetic activity is intense; the specific strength of the activity and the amount of energy injected into the thermosphere depend on factors such as the duration of the coronal mass ejection and the integral strength of the southward component of the interplanetary magnetic field. In summary, this paper establishes a method for inverting orbital decay that does not rely on onboard accelerometers but instead uses only precise orbital data. This method can be directly applied to orbital prediction, collision avoidance, and mission lifetime assessment for low-Earth orbit spacecraft during the peak year of Solar Activity Cycle 25.
